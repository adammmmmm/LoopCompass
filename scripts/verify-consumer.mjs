...
function parseManifestFiles(text) {
  ...
  const headers = [
    ...
    /^release: https:\/\/github\.com\/adammmmmm\/LoopCompass\/releases\/tag\/v\S+$/,
    /^commit: (?:[0-9a-f]{40,64}|unknown)$/,
    /^skill_schema: \d+$/,
    /^policy_version: \d+$/,
    /^state_schema: \d+$/,
    /^minimum_policy_version: \d+$/,
    /^files:$/,
  ];
  if (
    lines.length <= headers.length ||
    headers.some((pattern, index) => !pattern.test(lines[index] || ""))
  ) {
    return null;
  }
  const files = new Map();
  for (const rawLine of lines.slice(headers.length)) {
...
function validateRegularTree(root) {
  function visit(directory) {
    ...
    for (const name of readdirSync(directory)) {
      ...
      if (stat.isDirectory() && !stat.isSymbolicLink()) visit(candidate);
      else if (!stat.isFile() || stat.isSymbolicLink()) {
        throw new Error("non-regular filesystem node");
      }
    }
    const after = lstatSync(directory);
    if (!sameFile(before, after) || after.isSymbolicLink()) {
