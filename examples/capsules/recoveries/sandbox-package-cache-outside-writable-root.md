...
Point the package manager cache at a directory inside the sandbox writable area before install.
Do not use the redirect to mask a resolver or package failure.

## Verification

Lint, typecheck, and unit tests completed green after redirecting the cache on the same host class.
